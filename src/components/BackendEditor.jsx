...
const BackendEditor = ({ 
  ...
  const handleRunScript = () => {
    const res = backendEngine.execute(code);
    setResult(res);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%' }}>
      {/* Backend Code Terminal Container */}
      <div style={{ position: 'relative' }}>
        ...
        >
          <span style={{ fontFamily: 'var(--font-pixel)', fontSize: '0.7rem', color: '#ef4444' }}>
            ⚙️ SERVER FORTRESS IDE (Node.js / Express)
          </span>
          <span style={{ fontSize: '0.65rem', color: '#94a3b8' }}>
            Node v20.11.0 / Express 4.18
          </span>
        </div>
        ...
        <textarea
          ...
          placeholder="// Write your Backend Node.js code here..."
          data-cursor="text"
          rows={7}
          style={{
            width: '100%',
      ...
      {currentHintIndex >= 0 && (
        <div style={{ backgroundColor: 'rgba(245, 158, 11, 0.15)', border: '2px solid var(--accent-gold)', padding: '0.75rem', borderRadius: '6px', fontSize: '0.8rem', color: '#fef08a' }}>
          🔎 <strong>Backend Hint {currentHintIndex + 1}:</strong> {hintList[currentHintIndex]}
        </div>
      )}

      {/* Output Console Window */}
      {result && (
        <div style={{ backgroundColor: '#090d16', border: `2px solid ${result.success ? '#ef4444' : '#f87171'}`, borderRadius: '8px', padding: '1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            <span style={{ color: result.success ? '#ef4444' : '#f87171', fontFamily: 'var(--font-pixel)', fontSize: '0.7rem' }}>
              {result.success ? `✓ HTTP ${result.status} RESPONSE RECEIVED` : '❌ BACKEND SERVER ERROR'}
            </span>
          </div>
          ...
          {result.success ? (
            <div>
              ...
              <div style={{ backgroundColor: '#020617', padding: '0.75rem', borderRadius: '4px', border: '1px solid #1e293b', fontFamily: 'Consolas, monospace', fontSize: '0.85rem', color: '#fca5a5' }}>
                {result.output.length === 0 ? (
                  <span style={{ color: '#64748b', fontStyle: 'italic' }}>Server responded with empty payload.</span>
                ) : (
                  result.output.map((line, i) => (
                    <div key={i} style={{ lineHeight: 1.4, whiteSpace: 'pre-wrap' }}>{line}</div>
                  ))
                )}
              </div>
            </div>
