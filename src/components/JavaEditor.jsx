...
const JavaEditor = ({ 
  ...
  const handleRunScript = () => {
    const res = javaEngine.execute(code);
    setResult(res);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        >
          <span style={{ fontFamily: 'var(--font-pixel)', fontSize: '0.7rem', color: '#f59e0b' }}>
            ☕ JAVA KINGDOM IDE (.java)
          </span>
      ...
      {currentHintIndex >= 0 && (
        <div style={{ backgroundColor: 'rgba(245, 158, 11, 0.15)', border: '2px solid var(--accent-gold)', padding: '0.75rem', borderRadius: '6px', fontSize: '0.8rem', color: '#fef08a' }}>
          🔎 <strong>Java Hint {currentHintIndex + 1}:</strong> {hintList[currentHintIndex]}
        </div>
      )}

      {/* Output Console Window */}
      {result && (
        <div style={{ backgroundColor: '#090d16', border: `2px solid ${result.success ? '#f59e0b' : '#ef4444'}`, borderRadius: '8px', padding: '1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            <span style={{ color: result.success ? '#f59e0b' : '#ef4444', fontFamily: 'var(--font-pixel)', fontSize: '0.7rem' }}>
              {result.success ? '✓ JAVA CODE COMPILED & EXECUTED' : '❌ JAVA COMPILATION ERROR'}
            </span>
          </div>
          ...
          {result.success ? (
            <div>
              ...
              <div style={{ backgroundColor: '#020617', padding: '0.75rem', borderRadius: '4px', border: '1px solid #1e293b', fontFamily: 'Consolas, monospace', fontSize: '0.85rem', color: '#fbbf24' }}>
