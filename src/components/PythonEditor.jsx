...
const PythonEditor = ({ 
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', width: '100%' }}>
      {/* Python Code Terminal Container */}
      <div style={{ position: 'relative' }}>
        ...
        >
          <span style={{ fontFamily: 'var(--font-pixel)', fontSize: '0.7rem', color: '#22c55e' }}>
            🐍 PYTHON VALLEY SCRIPT EDITOR (.py)
          </span>
        ...
        <textarea
          ...
          placeholder="# Write your Python Valley code here..."
          data-cursor="text"
          rows={6}
          style={{
            width: '100%',
      ...
      {currentHintIndex >= 0 && (
        <div style={{ backgroundColor: 'rgba(245, 158, 11, 0.15)', border: '2px solid var(--accent-gold)', padding: '0.75rem', borderRadius: '6px', fontSize: '0.8rem', color: '#fef08a' }}>
          🔎 <strong>Python Hint {currentHintIndex + 1}:</strong> {hintList[currentHintIndex]}
        </div>
      )}

      {/* Output Console Window */}
      {result && (
        <div style={{ backgroundColor: '#090d16', border: `2px solid ${result.success ? '#22c55e' : '#ef4444'}`, borderRadius: '8px', padding: '1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
            <span style={{ color: result.success ? '#22c55e' : '#ef4444', fontFamily: 'var(--font-pixel)', fontSize: '0.7rem' }}>
              {result.success ? '✓ SCRIPT EXECUTED SUCCESSFULLY' : '❌ EXECUTION ERROR'}
            </span>
          </div>
          ...
          {result.success ? (
            <div>
              ...
              <div style={{ backgroundColor: '#020617', padding: '0.75rem', borderRadius: '4px', border: '1px solid #1e293b', fontFamily: 'Consolas, monospace', fontSize: '0.85rem', color: '#38bdf8' }}>
                {result.output.length === 0 ? (
                  <span style={{ color: '#64748b', fontStyle: 'italic' }}>Script completed with 0 printed outputs.</span>
                ) : (
