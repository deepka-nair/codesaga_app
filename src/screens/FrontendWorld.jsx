...
import { FRONTEND_CURRICULUM, getFrontendChapterById } from '../data/frontendCurriculum';
...
const FrontendWorld = () => {
  ...
  const activeChapter = getFrontendChapterById(activeChapterId);
  ...
  useEffect(() => {
    if (routeChapterId) {
      ...
      const ch = getFrontendChapterById(targetCh);
  ...
  useEffect(() => {
    ...
    updateFrontendProgress(activeChapterId, activeMissionIndex);
  }, [activeMissionIndex, activeChapterId, currentMission.id, completedMissions]);

  const handleSelectChapter = (chId) => {
    if ((isGuest && chId > 1) || (!isDev && chId > 1 && !feCompletedChapters.includes(chId - 1))) {
    ...
    const targetCh = getFrontendChapterById(chId);
    setActiveMissionIndex(getResumedMissionIndex(targetCh));
  };


  const handleFrontendResult = (userResult, codeData) => {
    if (!userResult.success) {
      setIsMissionSolved(false);
      return;
    }

    const reqValidation = frontendEngine.validateRequirements(
      codeData.htmlCode, 
      codeData.cssCode, 
      codeData.jsCode, 
      currentMission
  ...
  const handleNextMission = () => {
    ...
    if (activeMissionIndex < activeChapter.missions.length - 1) {
      ...
      updateFrontendProgress(activeChapterId, nextIdx);
    } else {
      if (isGuest) {
        setShowGuestUnlockModal(true);
        return;
  ...
  const handlePrevMission = () => {
    ...
    if (activeMissionIndex > 0) {
      ...
      updateFrontendProgress(activeChapterId, prevIdx);
    }
  };
  ...
  const handleModalContinue = () => {
    ...
    if (nextChapterId <= 12) {
      ...
      updateFrontendProgress(nextChapterId, 0);
    } else {
      navigate('/worlds');
    }
  };

  const feCompletedChapters = frontendProgress?.completedChapters || [];
  ...
  const isCurrentChapterLocked = !isDev && (isGuestChapterLocked || (activeChapterId > 1 && !feCompletedChapters.includes(activeChapterId - 1)));
      ...
      <div style={{ minHeight: '100vh', backgroundColor: '#0E2526', padding: '4rem 1.5rem', textAlign: 'center', color: '#F6F4EB' }}>
        <PixelPanel style={{ maxWidth: '600px', margin: '0 auto', borderTop: '4px solid #f59e0b', backgroundColor: '#173536' }}>
          ...
          <p style={{ color: '#A8B8B4', fontSize: '0.9rem', marginBottom: '1.5rem', lineHeight: 1.5 }}>
            Chapters 2+ are locked for Guest sessions. Create a free detective account or log in to unlock all 12 Frontend chapters, earn certificates, and save your progress to the cloud!
          </p>
      ...
      <div style={{ minHeight: '100vh', backgroundColor: '#0E2526', padding: '4rem 1.5rem', textAlign: 'center', color: '#F6F4EB' }}>
        <PixelPanel style={{ maxWidth: '600px', margin: '0 auto', borderTop: '4px solid #ef4444', backgroundColor: '#173536' }}>
          ...
          <h2 style={{ color: '#ef4444', fontFamily: 'var(--font-pixel)', fontSize: '1.5rem', marginBottom: '0.75rem' }}>
            {isChapLock ? `FRONTEND CHAPTER ${activeChapterId} IS LOCKED` : `FRONTEND MISSION ${activeMissionIndex + 1} IS LOCKED`}
          </h2>
          <p style={{ color: '#A8B8B4', fontSize: '0.9rem', marginBottom: '1.5rem', lineHeight: 1.5 }}>
            {isChapLock 
          ...
          <GameButton 
            onClick={() => {
              if (isChapLock) {
                const lastUnlocked = Math.max(1, ...(feCompletedChapters.map(c => c + 1)), 1);
                setActiveChapterId(lastUnlocked);
                const ch = getFrontendChapterById(lastUnlocked);
                setActiveMissionIndex(getResumedMissionIndex(ch));
              } else {
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            >
              ...
              <div>
                <h2 style={{ color: '#0ea5e9', margin: 0, fontSize: '1.4rem', fontFamily: 'var(--font-pixel)', textShadow: '2px 2px 0 #000' }}>
                  WEB CREATOR CITY RPG
                </h2>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '8px',
                    backgroundColor: isActive ? '#0ea5e9' : '#173536',
                    color: isActive ? '#0E2526' : '#F6F4EB',
                    border: '2px solid',
                    borderColor: isActive ? '#0284c7' : '#234a4b',
            ...
            <PixelPanel style={{ backgroundColor: '#173536', borderColor: '#234a4b', borderTop: `4px solid ${activeChapter.color}` }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  {isMissionSolved && (
                    <span style={{ color: '#0ea5e9', fontFamily: 'var(--font-pixel)', fontSize: '0.75rem' }}>
                      ✓ SOLVED
                    </span>
                  )}
                </div>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                <CharacterTalk
                  characterId={activeChapterId % 2 === 1 ? 'm1' : 'f3'}
              ...
              {currentMission.conceptExplanation && (
                <div style={{ backgroundColor: '#0E2526', border: '1px solid #2a5a5c', borderRadius: '8px', padding: '1rem', marginBottom: '1.25rem' }}>
                  <h4 style={{ color: '#0ea5e9', fontSize: '0.8rem', marginBottom: '0.5rem', fontFamily: 'var(--font-pixel)' }}>
                    📖 FRONTEND BRIEFING & EXPLANATION
                  </h4>
              ...
              {['multiple_choice', 'code_matching', 'predict_output'].includes(currentMission.type) ? (
                ...
                <div>
                  <FrontendEditor
                    ...
                    initialHtml={currentMission.templateHtml || ''}
                    initialCss={currentMission.templateCss || ''}
                    initialJs={currentMission.templateJs || ''}
                    onExecuteResult={handleFrontendResult}
            ...
            <div style={{
              ...
              <PixelPanel style={{ maxWidth: '520px', width: '100%', backgroundColor: '#1e293b', borderTop: '4px solid var(--accent-gold)', textAlign: 'center' }}>
                ...
                <div style={{ backgroundColor: '#0f172a', padding: '1rem', borderRadius: '8px', border: '1px solid #334155', textAlign: 'left', fontSize: '0.85rem', marginBottom: '1.5rem', color: '#f8fafc' }}>
                  <div style={{ marginBottom: '0.4rem' }}>✓ Full Web Creator & Multi-World Programming Chapters</div>
