...
import { PYTHON_CURRICULUM, getPythonChapterById } from '../data/pythonCurriculum';
...
const PythonWorld = () => {
  ...
  const activeChapter = getPythonChapterById(activeChapterId);
  ...
  useEffect(() => {
    if (routeChapterId) {
      ...
      const ch = getPythonChapterById(targetCh);
  ...
  useEffect(() => {
    ...
    updatePythonProgress(activeChapterId, activeMissionIndex);
  ...
  const handlePythonResult = (userResult, code) => {
    ...
    if (currentMission.expectedOutput) {
      isCorrect = pythonEngine.compareOutputs(userResult.output, currentMission.expectedOutput);
    } else {
  ...
  const handlePrevMission = () => {
    ...
    if (activeMissionIndex > 0) {
      ...
      updatePythonProgress(activeChapterId, prevIdx);
    }
  };
  ...
  const handleModalContinue = () => {
    ...
    if (nextChapterId <= 12) {
      ...
      updatePythonProgress(nextChapterId, 0);
    } else {
      navigate('/worlds');
    }
  };

  const pyCompletedChapters = pythonProgress?.completedChapters || [];
      ...
      <div style={{ minHeight: '100vh', backgroundColor: '#0E2526', padding: '4rem 1.5rem', textAlign: 'center', color: '#F6F4EB' }}>
        <PixelPanel style={{ maxWidth: '600px', margin: '0 auto', borderTop: '4px solid #f59e0b', backgroundColor: '#173536' }}>
          ...
          <p style={{ color: '#A8B8B4', fontSize: '0.9rem', marginBottom: '1.5rem', lineHeight: 1.5 }}>
            Chapters 2+ are locked for Guest sessions. Create a free detective account or log in to unlock all 12 Python chapters, earn certificates, and save your progress to the cloud!
          </p>
      ...
      <div style={{ minHeight: '100vh', backgroundColor: '#0E2526', padding: '4rem 1.5rem', textAlign: 'center', color: '#F6F4EB' }}>
        <PixelPanel style={{ maxWidth: '600px', margin: '0 auto', borderTop: '4px solid #ef4444', backgroundColor: '#173536' }}>
          ...
          <h2 style={{ color: '#ef4444', fontFamily: 'var(--font-pixel)', fontSize: '1.5rem', marginBottom: '0.75rem' }}>
            {isChapLock ? `PYTHON CHAPTER ${activeChapterId} IS LOCKED` : `PYTHON MISSION ${activeMissionIndex + 1} IS LOCKED`}
          </h2>
          <p style={{ color: '#A8B8B4', fontSize: '0.9rem', marginBottom: '1.5rem', lineHeight: 1.5 }}>
            {isChapLock
          ...
          <GameButton
            onClick={() => {
              if (isChapLock) {
                const lastUnlocked = Math.max(1, ...(pyCompletedChapters.map(c => c + 1)), 1);
                setActiveChapterId(lastUnlocked);
                const ch = getPythonChapterById(lastUnlocked);
                setActiveMissionIndex(getResumedMissionIndex(ch));
              } else {
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            >
              ...
              <div>
                <h2 style={{ color: '#22c55e', margin: 0, fontSize: '1.4rem', fontFamily: 'var(--font-pixel)', textShadow: '2px 2px 0 #000' }}>
                  PYTHON VALLEY RPG
                </h2>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '8px',
                    backgroundColor: isActive ? '#22c55e' : '#173536',
                    color: isActive ? '#0E2526' : '#F6F4EB',
                    border: '2px solid',
                    borderColor: isActive ? '#16a34a' : '#234a4b',
            ...
            <PixelPanel style={{ backgroundColor: '#173536', borderColor: '#234a4b', borderTop: `4px solid ${activeChapter.color}` }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap', gap: '0.5rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  {isMissionSolved && (
                    <span style={{ color: '#22c55e', fontFamily: 'var(--font-pixel)', fontSize: '0.75rem' }}>
                      ✓ SOLVED
                    </span>
                  )}
                </div>
              ...
              {currentMission.conceptExplanation && (
                ...
                <div style={{ backgroundColor: '#0E2526', border: '1px solid #2a5a5c', borderRadius: '8px', padding: '1rem', marginBottom: '1.25rem' }}>
                  <h4 style={{ color: '#22c55e', fontSize: '0.8rem', marginBottom: '0.5rem', fontFamily: 'var(--font-pixel)' }}>
                    📖 PYTHON BRIEFING & EXPLANATION
                  </h4>
              ...
              {['multiple_choice', 'code_matching', 'predict_output'].includes(currentMission.type) ? (
                ...
                <div>
                  <PythonEditor
                    ...
                    onExecuteResult={handlePythonResult}
